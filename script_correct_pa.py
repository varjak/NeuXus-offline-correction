...
lfol = 'data/'
sfol = 'res/'
pairs = {'set': [dset], 'fol': [lfol], 'sub': [0], 'ses': [0], 'run': [2], 'art': ['pa']}
...
for f, file in enumerate(files):
    ...
    EEG = eeglab.tools.trim(EEG, event1='S  1', event2='S 12', timeshifts=[0, 0])
    ...
    corrector_pa = correct.PA(df, True, dfs, mfil, start_marker='S  1', stride=stride, min_wins=10, max_wins=20, min_hc=0.4, max_hc=1.5, margin=0.1, numba=False, filter_ecg=False)

    if make_plot:
        plot_stride = 10
        start_plot = True
        plotter = plot.Plot(xdur=2, xmargin=1, slide='fi', lines=[{'subplot': 'C3', 'ylim': [-100, 100], 'name': 'ds', 'fs': dfs, 'col': 'red', 'label': 'Downsampled'},
    ...
    while lim2 < len(df):
        chunk_list_ds = [df.iloc[lim1:lim2].copy(deep=True)]
        chunk_list_marker_ga = [df_markers.loc[chunk_list_ds[0].index[0]:chunk_list_ds[0].index[-1]]]
        # chunk_list_ecg = selector.update(chunk_list_ds, ecg_chan)  # If filtering ECG before, instead of inside PA correction
    ...
    # Save
    EEG = eeglab.tools.pack(file['filename'] + '_cor-paNeXoff', df_pa.transpose().to_numpy(), df_pa.index.to_numpy(), dfs, chans, types, latencies)  # '_cor-paLas-clear'
